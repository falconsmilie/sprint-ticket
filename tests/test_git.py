...
def test_safety_snapshot_detects_branch_head_and_staging_changes(tmp_path):
    repo = create_git_repo(tmp_path / "repo")
    repository = GitRepository(repo)
    snapshot = GitSafetySnapshot.capture(repository)

    assert snapshot.compare(repository) == ()

    (repository.path / "file.txt").write_text("staged\n", encoding="utf-8")
    run_git(repository.path, "add", "file.txt")
    assert "staging" in {violation.name for violation in snapshot.compare(repository)}

    run_git(repository.path, "commit", "-m", "change")
    assert "HEAD" in {violation.name for violation in snapshot.compare(repository)}

    run_git(repository.path, "checkout", "-b", "another-branch")
    assert "branch" in {violation.name for violation in snapshot.compare(repository)}
